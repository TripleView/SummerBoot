...
namespace SummerBoot.Core.Configuration.Parser
{
    public class YamlParser
    {
        ...
        public static IDictionary<string, string?> Parse(string input)
            => new YamlParser().Parser(input);
        private IDictionary<string, string> Parser(string input)
        {
            _data.Clear();
            _context.Clear();
        ...
        private void VisitYamlScalarNode(string context, YamlScalarNode yamlValue)
        {
            ...
            {
                throw new FormatException(string.Format("A duplicate key '{0}' was found.", currentKey));
            }
