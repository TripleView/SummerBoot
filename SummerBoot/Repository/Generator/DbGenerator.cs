...
namespace SummerBoot.Repository.Generator
{
    public class DbGenerator : IDbGenerator
    {
        ...
        public List<string> GenerateCsharpClass(List<string> tableNames, string classNameSpace, string namesapce = "")
        {
            ...
            foreach (var tempTableName in tableNames)
            {
                var tableName = tempTableName;
        ...
        public List<GenerateDatabaseSqlResult> GenerateSql(List<Type> types, Dictionary<Type, string> fieldTypeMappers = null, bool isForce = false)
        {
            ...
            foreach (var type in types)
            {
                ...
                if (dbTableInfo.FieldInfos.Any()&& !isForce)
                {
                    ...
                    if (item.Body.HasText() || item.Descriptions.Any() || item.FieldModifySqls.Any())
                    {
                        result.Add(item);
                    }
                }
