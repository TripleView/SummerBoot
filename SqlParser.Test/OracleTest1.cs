...
namespace SqlParser.Test
{
    public class OracleTest1: SqliteParser
    {
        ...
        public void TestOraclePage4()
        {
            ...
            var parser = new OracleParser();
            var result = parser.ParserPage(sql, 1, 10);
            Assert.Equal("select * from (select row_number() over (order by null ) pageNo, sbInner.* FROM (select id,a.name as fullname,a.city address from person a) sbInner) sbOuter where pageNo > :sbPageSize and pageNo <= :sbPageSkip + :sbPageSize", result.PageSql);
