...
namespace SummerBoot.Test.Oracle
{
    ...
    public class RepositoryTest : IClassFixture<TestClassFixture>
    {
        private readonly TestClassFixture _fixture;

        // 构造函数：注入夹具实例（整个测试类中共享同一个实例）
        public RepositoryTest(TestClassFixture fixture)
        {
            _fixture = fixture;
            _fixture.InitOracleDatabase();
        }
        ...
        {
            var tempSql = "";
            var dynamicParameter = new SummerBoot.Repository.Core.DynamicParameters();
            var debugSqlAction = (string sql, SummerBoot.Repository.Core.DynamicParameters d) =>
            {
                tempSql = sql;
                dynamicParameter = d;
            };
            InitService(debugSqlAction);
            var customerRepository = serviceProvider.GetService<ICustomerRepository>();
            //Test insert,update,get,delete 
            var name = "TestInsertAsync";
            var customer = new Customer() { Name = name };
            await customerRepository.InsertAsync(customer);
            Assert.Equal("insert into \"CUSTOMER\"(\"NAME\", \"AGE\", \"CUSTOMERNO\", \"TOTALCONSUMPTIONAMOUNT\", \"BIRTHDAY\") values(:NAME, :AGE, :CUSTOMERNO, :TOTALCONSUMPTIONAMOUNT, :BIRTHDAY)", tempSql);
            Assert.Equal(6, dynamicParameter.GetParamInfos.Count);
            var r3 = await customerRepository.FirstOrDefaultAsync();
            var r4 = await customerRepository.FirstAsync();
            Assert.Equal("select \"p1\".\"Id\", \"p1\".\"Name\", \"p1\".\"Age\", \"p1\".\"CustomerNo\", \"p1\".\"TotalConsumptionAmount\", \"p1\".\"BirthDay\" from (select \"p0\".\"ID\" as \"Id\", \"p0\".\"NAME\" as \"Name\", \"p0\".\"AGE\" as \"Age\", \"p0\".\"CUSTOMERNO\" as \"CustomerNo\", \"p0\".\"TOTALCONSUMPTIONAMOUNT\" as \"TotalConsumptionAmount\", \"p0\".\"BIRTHDAY\" as \"BirthDay\", ROW_NUMBER() over (order by \"p0\".\"ID\") as \"sbRowNo\" from \"CUSTOMER\" \"p0\") \"p1\" where ((\"p1\".\"sbRowNo\" > :y0) and (\"p1\".\"sbRowNo\" <= :y1))", tempSql);
            Assert.Equal(name, r3.Name);
        }
        ...
        private void InitService(Action<string, SummerBoot.Repository.Core.DynamicParameters> debugSqlAction = null)
        {
